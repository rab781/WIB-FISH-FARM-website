    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden mb-8">
        ...
        <div class="flex flex-col md:flex-row border-b border-gray-200 hover:bg-gray-50">
            <div class="md:w-6 p-4 flex items-center justify-center">
                <input type="checkbox" name="selected_items[]" value="{{ $item->id_keranjang }}" class="rounded border-gray-300 text-orange-600 focus:ring-orange-500">
            </div>
            ...
            <div class="w-full md:w-32 p-4 flex items-center justify-center">
                <form action="{{ route('keranjang.update', $item->id_keranjang) }}" method="POST" class="quantity-form flex items-center">
                    ...
                    <input type="number" name="jumlah" value="{{ $item->jumlah }}" min="1" max="99" class="w-12 h-8 border-t border-b border-gray-300 text-center focus:outline-none focus:ring-1 focus:ring-orange-500">
            ...
            <div class="w-full md:w-24 p-4 flex items-center justify-center">
                <form action="{{ route('keranjang.destroy', $item->id_keranjang) }}" method="POST" class="inline-block">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-gray-500 hover:text-red-600 focus:outline-none" title="Hapus">
