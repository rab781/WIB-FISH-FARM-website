            ...
            <div class="flex flex-col md:flex-row border-b border-gray-200 hover:bg-gray-50 transition-colors duration-200" data-price="{{ $item->ukuran && $item->ukuran->harga ? $item->ukuran->harga : $item->produk->harga }}" data-total="{{ $item->total_harga }}">
                ...
                <div class="w-full md:w-32 p-4 flex items-center justify-center">
                    <div class="flex items-center">
                        <button type="button" class="quantity-decrease w-8 h-8 flex items-center justify-center bg-gray-100 text-gray-600 rounded-l border border-gray-300 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-500" data-cart-id="{{ $item->id_keranjang }}">
                        ...
                        <button type="button" class="quantity-increase w-8 h-8 flex items-center justify-center bg-gray-100 text-gray-600 rounded-r border border-gray-300 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-500" data-cart-id="{{ $item->id_keranjang }}">
                ...
                <div class="w-24 p-4 flex flex-row md:flex-col items-center md:justify-center space-x-2 md:space-x-0 md:space-y-2">
                    <form action="{{ route('keranjang.destroy.post', $item->id_keranjang) }}" method="POST" class="delete-form">
                        @csrf
                        <button type="submit" class="delete-btn text-red-600 hover:text-red-900 focus:outline-none focus:ring-2 focus:ring-red-500 rounded p-1">
...
@section('scripts')
<script>
    console.log('CART SCRIPT LOADING - Enhanced Version');

    // Enhanced SweetAlert fallback with better error handling
    function showAlert(config) {
        try {
            ...
            console.warn('SweetAlert2 error, using fallback:', e);
        }
        
        // Enhanced native fallback
    ...
    // Enhanced cart item visual update
    function updateCartItem(checkbox) {
        console.log('Updating cart item visual state:', checkbox.checked, checkbox.value);
        
        const row = checkbox.closest('[data-total]');
        if (!row) {
            console.error('Cart row not found for checkbox');
            return;
        }
    ...
    // Enhanced total calculation with better error handling
        ...
        try {
            ...
            selectedItems.forEach(item => {
                ...
                if (row && row.dataset.total) {
                    ...
                    console.warn('Invalid cart item row found');
                }
            });
            ...
            console.log(`Total updated: ${itemCount} items, Rp ${total.toLocaleString('id-ID')}`);
        } catch (error) {
            console.error('Error updating totals:', error);
        }
    }

    // Enhanced quantity change handler with comprehensive error handling
    function handleQuantityChange(cartId, action) {
        console.log(`QUANTITY CHANGE: ${action} for cart ID ${cartId}`);
        ...
        if (action === 'increase') {
            ...
            if (currentValue > 1) {
                ...
                console.log('Confirming deletion for quantity 0');
                showAlert({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (deleteForm) {
                            ...
                            showAlert({
                                title: 'Error',
                                text: 'Tidak dapat menghapus item. Silakan refresh halaman.',
                                icon: 'error'
                            });
                        }
                    } else {
        ...
        // Proceed with AJAX update if value changed
        if (newValue !== currentValue) {
            performQuantityUpdate(cartId, currentValue, newValue, input);
        }
    }

    // Separate function for AJAX quantity update
    function performQuantityUpdate(cartId, originalValue, newValue, input) {
        console.log(`AJAX UPDATE: Cart ${cartId} from ${originalValue} to ${newValue}`);
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                showAlert({
                    title: 'Berhasil',
                    text: data.message || 'Kuantitas berhasil diperbarui',
                    icon: 'success',
                    timer: 1500,
        ...
        .catch(error => {
            ...
            let errorMessage = 'Terjadi kesalahan saat memperbarui kuantitas';
            if (error.message) {
            ...
            try {
                ...
                if (e.target.closest('.quantity-decrease')) {
                    ...
                    if (cartId) {
                        ...
                        console.error('Cart ID not found on decrease button');
                    }
                    return;
                }
                ...
                if (e.target.closest('.quantity-increase')) {
                    ...
                    if (cartId) {
                        ...
                        console.error('Cart ID not found on increase button');
                    }
                    return;
                }
                ...
                if (e.target.closest('#lanjutPembayaranBtn')) {
                    ...
                    setTimeout(() => {
                        submitCheckout(selectedItems, btn, originalText);
                    }, 100);
                    return;
                }
            } catch (error) {
                ...
                showAlert({
                    title: 'Error',
                    text: 'Terjadi kesalahan. Silakan refresh halaman.',
                    icon: 'error'
                });
            }
        });
    ...
    // Submit checkout form
    function submitCheckout(selectedItems, btn, originalText) {
        try {
            const form = document.createElement('form');
            form.method = 'POST';
