<!-- Customer Notification dropdown component -->
<div class="relative inline-block" x-data="{ open: false, unreadCount: 0 }" x-init="
    fetch('{{ route('notifications.unread-count') }}?for_admin=0')
        .then(response => response.json())
        .then(data => { unreadCount = data.count; })
        .catch(error => console.error('Error fetching notification count:', error));
">
    <!-- Notification button with counter -->
    <button @click="open = !open; if(open) { loadCustomerNotifications(); }"
            class="relative p-1 text-gray-600 hover:text-orange-600 focus:outline-none focus:text-orange-500 transition-colors"
        ...
        <span x-show="unreadCount > 0"
              x-text="unreadCount"
              class="absolute top-0 right-0 -mt-1 -mr-1 px-2 py-0.5 rounded-full text-xs font-medium bg-orange-500 text-white">
        </span>
    </button>
    ...
    <div x-show="open"
         ...
         class="absolute right-0 mt-2 w-[28rem] bg-pink-50 rounded-md shadow-lg py-1 z-50 max-h-[600px] overflow-y-auto"
         style="display: none;">

        <!-- Notification title header -->
        <div class="px-4 py-3 border-b border-pink-100">
            <h3 class="text-base font-medium text-gray-700">Notifikasi Baru Diterima</h3>
        </div>

        <div id="customer-notification-container" class="bg-pink-50">
            <!-- Loading indicator -->
            <div id="customer-notification-loading" class="p-4 text-center text-gray-500">
            ...
            <div id="customer-notification-list" class="divide-y divide-pink-100"></div>

            <!-- Empty state -->
            <div id="customer-notification-empty" class="p-6 text-center text-gray-500" style="display:none;">
        ...
        <div class="border-t border-pink-100 mt-2 pt-2 pb-2 px-4 text-center">
            <a href="{{ route('notifications.index') }}" class="text-orange-600 hover:text-orange-800 font-medium">
...
<script>
// Define the loadCustomerNotifications function in global scope to make it accessible
function loadCustomerNotifications() {
    const container = document.getElementById('customer-notification-container');
    const loading = document.getElementById('customer-notification-loading');
    const list = document.getElementById('customer-notification-list');
    const empty = document.getElementById('customer-notification-empty');
    ...
    fetch('{{ route('notifications.index') }}?format=json&for_admin=0', {
        method: 'GET',
        headers: {
    ...
    .then(data => {
        ...
        data.notifications.forEach(notification => {
            ...
            item.className = `p-4 hover:bg-pink-100 cursor-pointer transition duration-150 ${notification.is_read ? 'opacity-70' : 'bg-pink-50'}`;
            ...
            switch(notification.type) {
                ...
                case 'review':
                    ...
                    break;
                default:
                    iconHtml = `<div class="flex-shrink-0 h-14 w-14 rounded bg-pink-50 flex items-center justify-center">
                    ...
                    <div class="ml-4 flex-1">
                        ...
                        <div class="flex justify-between mt-1">
                            ...
                            ${!notification.is_read ?
                              `<button class="text-xs text-orange-600 hover:text-orange-800 mark-read" data-id="${notification.id}">Tandai dibaca</button>` : ''}
                        </div>
                    </div>
                </div>
            `;
