    ...
    <div class="fade-in">
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mb-4">
                ...
                <x-text-input id="name" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500" type="text" name="name" :value="old('name')" autofocus autocomplete="name" />
                @if($errors->has('name'))
            ...
            <div class="mb-4">
                ...
                <x-text-input id="email" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-orange-500 focus:ring-orange-500" type="email" name="email" :value="old('email')" autocomplete="username" />
                @if($errors->has('email'))
                    <div class="flex items-center mt-2 text-sm text-red-600 bg-red-100 border border-red-300 rounded px-2 py-1 shake">
                        <svg class="w-4 h-4 mr-1 text-red-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 1 1 3 12a9 9 0 0 1 18 0Z" /></svg>
                        {{$errors->first('email')}}
            ...
            <div class="mt-4 text-center">
                <a class="text-sm text-gray-600 hover:text-gray-900 underline rounded-md" href="{{ route('login') }}">
                    {{ __('Sudah Punya Akun ? Langung Login') }}
                </a>
            </div>
        </form>
    </div>
